...
class Translator(object):
    ...
    def trans_onesent(self, s, domain):

        trans_start = time.time()

        if self.search_mode == 0: trans = self.greedy.greedy_trans(s)
        elif self.search_mode == 1: (trans, ids), loss, attent_matrix = self.nbs.beam_search_trans(s, domain)
    ...
    def trans_samples(self, srcs, trgs, domain):
        ...
        for idx in range(len(srcs)):
            ...
            trans, _, attent_matrix = self.trans_onesent(s_filter, domain)
    ...
    def force_decoding(self, batch_tst_data, domain="IN"):
        ...
        for batch_idx in range(batch_count):
            ...
            _, attends = self.model(srcs, trgs[:-1], srcs_m, trgs_m[:-1], domain, isAtt=True, test=True)
    ...
    def single_trans_file(self, src_input_data, src_labels_fname=None, batch_tst_data=None, domain="IN"):
        ...
        if batch_tst_data is not None:
            ...
            fd_attent_matrixs, trgs = self.force_decoding(batch_tst_data, domain=domain)
        ...
        for bid in range(batch_count):
            ...
            for no in range(batch_srcs_LB.size(1)): # batch size, 1 for valid
                ...
                if src_labels_fname is not None:
                    ...
                    trans = []
                    for seg in segs:
                        seg_trans, ids, _ = self.trans_onesent(seg, domain)
                        words_cnt += len(ids)
                        trans.append(seg_trans)
                    # merge by order
                    trans = ' '.join(trans)
                else:
                    trans, ids, attent_matrix = self.trans_onesent(s_filter, domain)
    ...
    def trans_eval(self, valid_data, eid, bid, model_file, tests_data, domain="IN"):
        ...
        trans, alns = self.single_trans_file(valid_data, label_fname, domain=domain)
